...
func findMaxPath(triangle [][]int,x,y int)int{
    max:=len(triangle)
    if x==max-1{
        return triangle[x][y]
    } else {
        value:=triangle[x][y]
        a:=findMaxPath(triangle,x+1,y)
        b:=findMaxPath(triangle,x+1,y+1)
        if a>b{
            return a+value
        } else {
            return b+value
...
func main(){
    ...
    最大和路径
    从三角的最上，到达最下一行有多种路径，哪一种路径上的数的和最大？找到这个最大的和。
...
{4,62,98,27,23,9,70,98,73,93,38,53,60,4,23}}
    fmt.Println(findMaxPath(arr,0,0))
